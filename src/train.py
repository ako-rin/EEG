...
from LibEER.config.setting import preset_setting, set_setting_by_args, Setting, seed_sub_dependent_front_back_setting
...
def main():
    # Build setting
    setting = Setting(dataset='seed_de_lds',  # Select the dataset
    ...
    for rridx, (data_i, label_i) in enumerate(zip(data, label), 1):
        ...
        for ridx, (train_indexes, test_indexes, val_indexes) in enumerate(zip(tts['train'], tts['test'], tts['val']), 1):
            ...
            # 每一轮/每一折都新建一个全新的 PGCN 实例（和新的优化器/调度器），从随机初始化开始训练
